...
void init_simulation(uint32_t np)
{
   ...
   for(uint16_t i = 0; i < np; i++){
      ...
      hd->all_patients[i].treated = PTHREAD_COND_INITIALIZER;
   }
}
...
bool nurse_iteration()
{
   ...
   if(patient == MAX_PATIENTS) return false;
...
bool doctor_iteration()
{
   ...
   cond_broadcast(&hd->all_patients[patient].treated);
...
void patient_wait_end_of_consultation(int id)
{
   ...
   while(hd->all_patients[id].done == 0)
      cond_wait(&hd->all_patients[id].treated, &hd->all_patients[id].accessCR);
...
void* call_doctor(void* args){
   int32_t *patients_treated = (int32_t *)args;
   while(true){
      bool treat = doctor_iteration();
...
void* call_nurse(void* args){
   int32_t *patients_treated = (int32_t *)args;
   while(true){
      bool treat = nurse_iteration();
...
{
   uint32_t npatients = 100;  ///< number of patients
   uint32_t nnurses = 10;    ///< number of triage nurses
   uint32_t ndoctors = 10;   ///< number of doctors
   ...
   pthread_t doctorThreads[ndoctors], nurseThreads[nnurses], patientThreads[npatients];
   int patientArgs[npatients];
   int32_t nurseArgs[nnurses], doctorArgs[ndoctors];

   for(uint32_t i = 0; i < ndoctors; i++){
      doctorArgs[i] = 0;
      pthread_create(&doctorThreads[i], NULL, &call_doctor, &doctorArgs[i]);
   }

   for(uint32_t i = 0; i < nnurses; i++){
      nurseArgs[i] = 0;
      pthread_create(&nurseThreads[i], NULL, &call_nurse, &nurseArgs[i]);
   }

   for(uint32_t i = 0; i < npatients; i++){
      patientArgs[i] = i;
      pthread_create(&patientThreads[i], NULL, &call_patient, &patientArgs[i]);
   }

   // Wait for patients to exit hospital
   for(uint32_t i = 0; i < npatients; i++){
      pthread_join(patientThreads[i], NULL);
   }

   // Send message to terminate nurses
   for(uint32_t i = 0; i < nnurses; i++){
      insert_pfifo(&hd->triage_queue, MAX_PATIENTS, 1);
   }

   // Send message to terminate doctors
   for(uint32_t i = 0; i < ndoctors; i++){
   ...
   for(uint32_t i = 0; i < nnurses; i++){
      ...
      fprintf(stdout, "Nurse %d finished shift, treating %d patients\n", i, nurseArgs[i]);
   }

   for(uint32_t i = 0; i < ndoctors; i++){
      pthread_join(doctorThreads[i], NULL);
      fprintf(stdout, "Doctor %d finished shift, treating %d patients\n", i, doctorArgs[i]);
   }
